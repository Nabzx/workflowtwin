...
export function EngineeringPage() {
  ...
  return (
    <div className="page-container engineering-page">
      ...
      <section><SectionHeading eyebrow="Supported product path" title="Evidence lineage from events to rollback" description="Fingerprints link prepared artefacts; the interface never exposes raw hidden labels or local paths." /><div className="engineering-flow" id="product-path" role="region" aria-label="Product evidence lineage" tabIndex={0}>{data.product_path.map((stage, index) => <div key={stage}><span>{String(index + 1).padStart(2, "0")}</span><strong>{stage}</strong>{index < data.product_path.length - 1 && <ArrowRight />}</div>)}</div></section>
